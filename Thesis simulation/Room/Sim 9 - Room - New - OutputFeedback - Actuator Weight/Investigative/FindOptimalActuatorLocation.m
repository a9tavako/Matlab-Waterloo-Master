...
HeatXPos = 0.1:xRes:2; %possible sensor x coordinates
HeatYPos = 0.1:yRes:2; %possible sensor y coordinates
h2scores = zeros(length(HeatXPos),length(HeatYPos));

for counterX = 1:length(HeatXPos)
    for counterY = 1:length(HeatYPos)
    if HeatXPos(counterX) < 1 && HeatYPos(counterY) > 1 
        h2scores(counterX,counterY) = NaN;
        continue; %if in the top left area, skip cause it's outside the domain
    end
    heatX   = HeatXPos(counterX); 
    heatY   = HeatYPos(counterY); 
...
[p,e,t,boundaryPointsIndex,windowPointsIndex,heaterPointsIndex,sensorPointsIndex] = makeHandMadeMesh(xRes,yRes,...
    ...
    [h2scores(counterX,counterY),X] = calculateOptimalH2ControlCost(A,B1,B2,...
...
figure(3)
clf
contourf(HeatXPos,HeatYPos,h2scores')                  ; % plotting the results, 2D color map
contourcbar                                          ;
